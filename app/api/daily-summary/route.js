...
async function sendSummaryEmail({ parentEmail, studentName, wordCount }) {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { name: 'ท่องศัพท์วันละ 10 คำ', email: senderEmail },
...
export async function GET(request) {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      throw new Error('Supabase URL/anon key env vars are missing');
    }
    ...
    if (accErr) {
      console.error('accounts query failed:', accErr);
      return NextResponse.json({ error: accErr.message, step: 'accounts query' }, { status: 500 });
    }
    ...
    for (const acc of accounts || []) {
      ...
      if (countErr) {
        console.error('progress query failed for account', acc.id, countErr);
      ...
      if (count && count > 0) {
        try {
          ...
          console.error('send email failed for account', acc.id, e);
    ...
    console.error('daily-summary route crashed:', e);
