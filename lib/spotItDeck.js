// ---------- คลังสัญลักษณ์ 57 คำ (word + emoji + ความหมายไทย) ----------
...
export function getPlayDeck(cardCount = 32) {
  const fullDeck = generateFullDeck();
  const picked = shuffle(fullDeck).slice(0, cardCount);
  // แปลง symbol id เป็นข้อมูลคำเต็ม (พร้อมลิงก์รูป Twemoji) พร้อมสุ่มลำดับการวางในแต่ละใบให้ดูเป็นธรรมชาติ
  return picked.map((symbolIds) =>
    shuffle(symbolIds).map((id) => ({
      id,
      ...SYMBOLS[id],
      imageUrl: emojiToImageUrl(SYMBOLS[id].emoji),
    }))
  );
}
