...
// n=7 (จำนวนเฉพาะ) -> สัญลักษณ์/ใบ = n+1 = 8, จำนวนการ์ดทั้งหมด = n²+n+1 = 57
// (แผน B: กลับไป 8 สัญลักษณ์/ใบ เพื่อรองรับจำนวนใบ/คนตามที่ต้องการ — กลไกเดาความหมายทำให้ความยากยังคงอยู่แม้สัญลักษณ์/ใบเยอะขึ้น)
...
// สุ่มว่าสัญลักษณ์ตำแหน่งนี้จะโชว์เป็นอะไร — ระดับ 1 สุ่มได้ทั้งรูป/อังกฤษ/ไทย, ระดับ 2-3 สุ่มแค่อังกฤษ/ไทย (ไม่มีรูปที่มีความหมาย)
function pickVariant(level) {
  if (level === 1) {
    const opts = ['image', 'en', 'th'];
    return opts[Math.floor(Math.random() * opts.length)];
  }
  return Math.random() < 0.5 ? 'en' : 'th';
}

export function getPlayDeck(symbolPool, cardCount = 24, level = 1, n = ORDER) {
  const fullDeck = generateFullDeck(n);
  const picked = shuffle(fullDeck).slice(0, Math.min(cardCount, fullDeck.length));
  return picked.map((symbolIds) => {
    const shuffled = shuffle(symbolIds);
    // ตำแหน่งแรกหลังสุ่ม (index 0) = ตำแหน่งกลางวงที่ symbolLayout วาง
    // บังคับให้เป็น 'image' เสมอ (emoji น่ารักตรงกลาง) — ระดับ 1 ตรงกับความหมายคำอยู่แล้ว
    // ระดับ 2-3 ใช้ decorative icon ที่สุ่มไว้ตอน buildSymbolPool (ไม่อิงความหมาย ตามที่ต้องการ)
    // ตำแหน่งที่เหลือ (รอบวง 7 ตำแหน่ง) สุ่ม variant ตามกลไกเดิมของแต่ละระดับ
    return shuffled.map((id, idx) => ({
      ...symbolPool[id],
      variant: idx === 0 ? 'image' : pickVariant(level),
    }));
  });
}
